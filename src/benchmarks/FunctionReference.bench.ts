...
import { resolve, ModuleResolutionError, ReferencedSymbolIsNotAFunction } from '../FunctionReference.js'
...
bench('FunctionReference.resolve: valid + call returned fn', async () => {
  ...
  // pipe with 2 args
  return (fn as Function)((x: number) => x + 1, (x: number) => x * 2)
})
...
console.log(`[memory] 1k FunctionReference.resolve (cached): ${((heapAfter - heapBefore) / 1024 / 1024).toFixed(2)} MB`)
void resolvedFns
