...
bench("ErrorChain.find: match last (depth 3)", () => {
  return find((_e) => (_e as Error).message === "Error level 0", chain3);
});

bench("ErrorChain.find: no match (depth 10)", () => {
  return find((_e) => false, chain10);
});
...
console.log(
  `[memory] 10k error chains (depth 5): ${((heapAfter - heapBefore) / 1024 / 1024).toFixed(2)} MB`,
);
void chains;
