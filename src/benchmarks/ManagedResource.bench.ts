...
const simpleResourceDef = {
  ...
  dispose: (r: SimpleResource) => { r.closed = true },
}

const asyncResourceDef = {
  factory: async (): Promise<SimpleResource> => ({ value: 42, closed: false }),
  dispose: async (r: SimpleResource) => { r.closed = true },
}

// --- Warm-up ---
{
  for (let i = 0; i < 100_000; i++) {
    await withResource(simpleResourceDef, async (r) => { void r.value })
  }
...
bench('withResource: sync factory + sync dispose + noop fn', async () => {
  return withResource(simpleResourceDef, async (r) => { void r.value })
})

bench('withResource: async factory + async dispose', async () => {
  return withResource(asyncResourceDef, async (r) => { void r.value })
})
...
bench('withResource: sequential 5 resources', async () => {
  let sum = 0
  for (let i = 0; i < 5; i++) {
    await withResource(simpleResourceDef, async (r) => { sum += r.value })
  }
  return sum
})
...
console.log(`[memory] 10k withResource calls: ${((heapAfter - heapBefore) / 1024 / 1024).toFixed(2)} MB`)
