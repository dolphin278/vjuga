 ...
 *   toonStringify: Turbofan (top tier). toonPar/toonTabStr: Maglev only.
 *   The generated parse code doesn't reach Turbofan because it exceeds
 *   the inlining budget: each field emits 5-15 bytecodes (startsWith,
 *   slice, type coerce, error branch, assign), nested objects multiply
 *   the count, and string operations (split, indexOf, regex match) create
 *   polymorphic call sites. Turbofan bails out on the combined code size.
 *   To reach Turbofan: extract field-parse patterns into shared helper
 *   functions captured as refs (e.g. `_parseStr(lines, li, key, pad)`),
 *   replace `lines = input.split("\n")` with streaming indexOf-based
 *   line scanning, and simplify path expressions. This would roughly
 *   halve the generated bytecode count and make each helper individually
 *   Turbofan-eligible.
 *
 * @example
 * ```ts
